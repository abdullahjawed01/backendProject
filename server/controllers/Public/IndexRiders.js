...
router.post("/register", async (req, res) => {
  try {
    let { RidersName, email, phone, password, address,vehicleType,licenseNumber } =req.body;

    // duplicate user 
    let duplicate = await ridersModel.findOne({email})
    if(duplicate){
    ...
    let customer = {
        ...
        await ridersModel.create(customer)
    // send email 
...
router.get("/email-verify/:emailToken", async (req, res) => {
  try {
    ...
    let user = await ridersModel.findOne({
...
router.post("/login",async(req,res)=>{
    try {
        ...
        let check = await ridersModel.findOne({email})
        if(!check){
            return res.status(401).json("user not found")
        }
        
        let user = await ridersModel.findOne({ email });
    if (!user) {
...
router.post("/forgetpass",async(req,res)=>{
  try {
    ...
    let one = await ridersModel.findOne({email})
    if(!one){
